...
// Lineáris kereső példány
$searchEngine = new LinearFuzzySearch($sampleWords);
...
<head>
    <meta charset="UTF-8">
    <title>Egyszerű Fuzzy Kereső</title>
    <style>
        body {
...
<body>
    <h1>Egyszerű Fuzzy Kereső – Lineáris mód</h1>
    <form method="post" action="">
        <label for="query">Keresett szó/szavak:</label>
        ...
        <?php if (!empty($results)): ?>
            <ul>
                <?php foreach ($results as $r): ?>
                    <li class="result">
                        <strong>Szó:</strong> <?php echo htmlspecialchars($r['word']); ?>,
                        <strong>Távolság:</strong> <?php echo $r['distance']; ?>
                    </li>
                <?php endforeach; ?>
            </ul>
        <?php else: ?>
