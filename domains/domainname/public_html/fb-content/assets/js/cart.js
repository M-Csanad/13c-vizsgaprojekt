import {
  ...
  setupNumberField,
} from "../../fb-products/js/numberfield.js";
...
class Cart {
  ...
  bindEvents() {
    ...
    if (this.floatingAddButton) {
      this.floatingAddButton.addEventListener("click", (e) => this.add(e));
    }
    ...
    this.cartContainer.addEventListener("click", async (e) => {
      ...
      if (e.target.closest(".number-field-add")) {
        ...
        await this.changeCount(
          ...
          currentElement.getAttribute("max")
        );
        return;
      }
  ...
  async add(e, url = null) {
    let qty = this.quantityInput ? Number(this.quantityInput.value) : 1;
    if (e && e.target.closest(".floating-add-to-cart")) {
      const floatingInput = document.querySelector(".floating-qty-value");
      if (floatingInput) {
        qty = Number(floatingInput.value);
      }
    }
  ...
  async changeCount(operation = "+", index, currentValue, maxValue) {
    const product = this.data[index];
    const productElement = Array.from(this.cartContainer.children).filter(
      (e) => e.nodeName != "HR"
    )[index];

    if (operation != "+" && operation != "-")
      throw new Error(
        "Ismeretlen művelet a changeCount függvényben: " + operation
      );

    const delta = operation === "-" ? -1 : 1;
    if (
      Number(currentValue) + delta > Number(maxValue) ||
      Number(currentValue) + delta < 1
    )
      return;
