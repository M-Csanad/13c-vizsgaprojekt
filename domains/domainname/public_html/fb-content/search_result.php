...
// Ellenőrizzük, van-e keresési eredmény
...
// Ha nincs találat
if (empty($productIds)) {
    echo "Nincsenek találatok a \"" . htmlspecialchars($searchQuery) . "\" kifejezésre.";
    exit;
}

// 2) Lekérjük a product_page link_slug-et
$placeholders = implode(',', array_fill(0, count($productIds), '?'));
$typeString = str_repeat('i', count($productIds));

$sqlPages = "
    SELECT product_id, link_slug
    FROM product_page
    WHERE product_id IN ($placeholders)
    ORDER BY product_id ASC
";
$resultPages = selectData($sqlPages, $productIds, $typeString);
if ($resultPages->isError()) {
    echo "Hiba a link_slug lekérdezésben: " . $resultPages->message;
    exit;
}
$linkRows = $resultPages->isEmpty() ? [] : $resultPages->message;
$linkMap = [];
foreach ($linkRows as $row) {
    $linkMap[$row['product_id']] = $row['link_slug'];
}

// 3) Lekérjük a képeket (thumbnail_image, secondary_image)
$sqlImages = "
    SELECT
...
";
$resultImages = selectData($sqlImages, $productIds, $typeString);
if ($resultImages->isError()) {
    echo "Hiba a képek lekérdezésében: " . $resultImages->message;
    exit;
}
$imageRows = $resultImages->isEmpty() ? [] : $resultImages->message;
$imageMap = [];
foreach ($imageRows as $r) {
    $thumbCleaned = preg_replace('/\.[a-zA-Z0-9]+$/', '', $r['thumbnail_image']);
    $secCleaned = preg_replace('/\.[a-zA-Z0-9]+$/', '', $r['secondary_image']);
    $imageMap[$r['product_id']] = [
        'thumbnail_image' => $thumbCleaned,
        'secondary_image' => $secCleaned,
    ];
}

// 4) Összerakunk egy végleges $products tömböt
$products = [];
foreach ($searchResults as $item) {
    $p = $item['product'];
    $pid = (int) $p['id'];

    // Becsattoljuk a linket, ha van
    $p['link_slug'] = $linkMap[$pid] ?? null;

    // Ha van kép
    if (isset($imageMap[$pid])) {
        $p['thumbnail_image'] = $imageMap[$pid]['thumbnail_image'];
        $p['secondary_image'] = $imageMap[$pid]['secondary_image'];
    } else {
        // alapértelmezett
        $p['thumbnail_image'] = '/path/to/default_thumbnail';
        $p['secondary_image'] = '/path/to/default_secondary';
    }
    $products[] = $p;
}

// 5) Ha van subcategory adatról gondoskodni akarsz, bővítsd ki a kódot.
//    Ha nem kell subcategory, beégetheted pl. dummy adatokat,
//    vagy cseréld " – " a HTML-ben.
    ...
    <main>
        <section class="filters">
            ...
            <div class="product-count">
                <?= count($products) ?> termék találat a "<?= htmlspecialchars($searchQuery) ?>" kifejezésre
            </div>
            <div class="filter-logo">
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-funnel" viewBox="0 0 16 16">
                    <path d="M1.5 1.5A.5.5 0 0 1 2
                        1h12a.5.5 0 0 1 .5.5v2a.5.5 0 0
                        1-.128.334L10 8.692V13.5a.5.5 0
                        0 1-.342.474l-3 1A.5.5 0 0
                        1 6 14.5V8.692L1.628
                        3.834A.5.5 0 0 1 1.5
                        3.5zm1 .5v1.308l4.372 4.858A.5.5
                        0 0 1 7 8.5v5.306l2-.666V8.5a.5.5
                        0 0 1 .128-.334L13.5 3.308V2z" />
                </svg>
            </div>
            ...
            <div class="filter-close">
                <svg xmlns="http://www.w3.org/2000/svg" class="bi bi-x-lg" viewBox="0 0 16 16">
                    <path d="M2.146 2.854a.5.5
                        0 1 1 .708-.708L8 7.293l5.146-5.147a.5.5
                        0 0 1 .708.708L8.707
                        8l5.147 5.146a.5.5
                        0 0 1-.708.708L8
                        8.707l-5.146
        ...
        <div class="products-wrapper">
            <section class="products">
                <div class="topbar-wrapper">
                    <div class="info">
                        <!--
                            Ha subcategoryData kellene, ide illeszthetsz valamit,
                            pl. "Keresési eredmények" fixen:
                        -->
                    ...
                    <div class="buttons">
                        <button class="filter-open">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-funnel"
                                viewBox="0 0 16 16">
                                <path d="M1.5 1.5A.5.5
                                    0 0 1 2 1h12a.5.5 0 0
                                    1 .5.5v2a.5.5 0 0
                                    1-.128.334L10
                                    8.692V13.5a.5.5 0 0
                                    1-.342.474l-3
                                    ...
                                    1 6 14.5V8.692L1.628
                                    3.834A.5.5 0 0 1 1.5
                                    3.5zm1 .5v1.308l4.372
                                    4.858A.5.5 0 0 1 7
                                    8.5v5.306l2-.666V8.5a.5.5
                                    0 0 1 .128-.334L13.5
                                    3.308V2z" />
                            </svg>
                            Szűrés
                    ...
                    <?php foreach ($products as $product): ?>
                        <div class="card">
                            <div class="card-image">
                                <a href="<?php
                                // /slug, ha van, különben /product/id
                                echo $product['link_slug']
                                    ? '/' . htmlspecialchars($product['link_slug'])
                                ...
                                ?>">
                                    ...
                                    <picture>
                                        <?php foreach ($resolutions as $r): ?>
                                            <source type="image/avif"
                                                srcset="<?= $product["thumbnail_image"] ?>-<?= $r ?>px.avif 1x"
                                                media="(min-width: <?= $r ?>px)">
                                            <source type="image/webp"
                                                srcset="<?= $product["thumbnail_image"] ?>-<?= $r ?>px.webp 1x"
                                                media="(min-width: <?= $r ?>px)">
                                            <source type="image/jpeg"
                                                srcset="<?= $product["thumbnail_image"] ?>-<?= $r ?>px.jpg 1x"
                                                media="(min-width: <?= $r ?>px)">
                                        <?php endforeach; ?>
                                        <!-- Fallback -->
                                        <img src="<?= $product["thumbnail_image"] ?>-<?= end($resolutions) ?>px.webp"
                                    ...
                                    <picture class="secondary">
                                        <?php foreach ($resolutions as $r): ?>
                                            <source type="image/avif"
                                                srcset="<?= $product["secondary_image"] ?>-<?= $r ?>px.avif 1x"
                                                media="(min-width: <?= $r ?>px)">
                                            <source type="image/webp"
                                                srcset="<?= $product["secondary_image"] ?>-<?= $r ?>px.webp 1x"
                                                media="(min-width: <?= $r ?>px)">
                                            <source type="image/jpeg"
                                                srcset="<?= $product["secondary_image"] ?>-<?= $r ?>px.jpg 1x"
                                                media="(min-width: <?= $r ?>px)">
                                        <?php endforeach; ?>
                                        <!-- Fallback -->
                                        <source type="image/jpeg" srcset="<?= $product["secondary_image"] ?>.jpg 1x"
                                            media="(min-width: 0px)">
                                        <img src="<?= $product["secondary_image"] ?>.jpg"
                                            alt="<?= htmlspecialchars($product['name']) ?>" loading="lazy"
                                            sizes="(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 33vw">
                                    </picture>
                                </a>
                                <div class="button-wrapper">
                                    <button class="quick-add" id="<?= htmlspecialchars(format_str($product["name"])); ?>">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                            ...
                                            <path d="M8 1a2.5 2.5 0 0
                                                1 2.5 2.5V4h-5v-.5A2.5
                                                2.5 0 0 1 8 1m3.5
                                                3v-.5a3.5 3.5 0
                                                1 0-7 0V4H1v10a2 2
                                                0 0 0 2 2h10a2 2 0 0
                                                0 2-2V4zM2 5h12v9a1
                                                1 0 0 1-1
                                                1H3a1 1 0 0
                                                1-1-1z" />
