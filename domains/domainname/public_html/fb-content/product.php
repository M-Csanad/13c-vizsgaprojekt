...
if (is_array($result->message)) {
  $benefits = array_filter($result->message, function ($e) {
    return $e["benefit"] == 1;
  });
  $side_effects = array_filter($result->message, function ($e) {
    return $e["benefit"] == 0;
  });
}
...
if (is_array($reviews)) {
  ...
  $avgReview = array_sum(array_map(function ($e) {
    return $e["rating"];
  }, $reviews)) / $reviewNum;
}
...
if (count($relatedProducts) < $limit) {
  $result = selectData(
    "SELECT DISTINCT p.*, pp.link_slug,
      ...
      AND p.id NOT IN (" . implode(',', array_map(function ($p) {
      return $p['id'];
    }, $relatedProducts)) . ")
      GROUP BY p.id
      ORDER BY p.name
...
if (count($relatedProducts) < $limit) {
  $result = selectData(
    "SELECT DISTINCT p.*, pp.link_slug,
      ...
      LEFT JOIN review r ON p.id = r.product_id
      WHERE p.id != ?
      AND p.id NOT IN (" . (empty($relatedProducts) ? '-1' : implode(',', array_map(function ($p) {
      return $p['id']; }, $relatedProducts))) . ")
      GROUP BY p.id
      ORDER BY p.name
      LIMIT ?",
...
<head>
  ...
  <link rel="preload" href="/fb-content/assets/font/PlayfairDisplay-VariableFont_wght.woff2" as="font" type="font/woff2"
    crossorigin="anonymous" />
  <link rel="preload" href="/fb-content/assets/font/Roboto-Regular.woff2" as="font" type="font/woff2"
    crossorigin="anonymous" />
  ...
  <main class="body-main">
      ...
      <div class="wrapper">
        <div class="images">
          <?php foreach ($images as $index => $image): ?>
            <img src="<?= htmlspecialchars($image["uri"]); ?>" alt="Termékkép"
              class="image <?= htmlspecialchars(($index == 0) ? "active" : "") ?>"
              style="z-index: <?= htmlspecialchars(($index == 0) ? 100 : -1); ?>;" />
          <?php endforeach; ?>
        </div>
        <div class="navigator">
          <div class="navigator-images">
            <?php foreach ($images as $index => $image): ?>
              <img src="<?= htmlspecialchars($image["uri"]); ?>" alt="Navigációs kép" class="navigator-image"
                draggable="false" />
            <?php endforeach; ?>
          </div>
    ...
    <section class="product-main">
      ...
      <div class="breadcrumb">
        ...
        <a href="<?= htmlspecialchars(ROOT_URL . "/" . format_str($product["category_name"])); ?>"
          class="breadcrumb-item"><?= htmlspecialchars($product["category_name"]); ?></a>
        <div class="breadcrumb-splitter">/</div>
        <a href="<?= htmlspecialchars(ROOT_URL . "/" . format_str($product["category_name"]) . "/" . format_str($product["subcategory_name"])); ?>"
          class="breadcrumb-item"><?= htmlspecialchars($product["subcategory_name"]); ?></a>
        <div class="breadcrumb-splitter">/</div>
      </div>
      ...
      <div class="inline-container">
        <?php if (is_array($reviews)): ?>
          <div class="avg-review" aria-label="Átlagos értékelés" data-rating="<?= htmlspecialchars($avgReview) ?>"
            data-reviews="<?= htmlspecialchars($reviewNum) ?>"></div>
        <?php endif; ?>
        <div class="stock-indicator">
          ...
          <div class="stock-bar-container">
            <div
              class="stock-bar <?= $product["stock"] <= 0 ? 'out-of-stock' : ($product["stock"] > 10 ? 'high' : ($product["stock"] > 5 ? 'medium' : 'low')) ?>"
              style="width: <?= $product["stock"] <= 0 ? '100' : min(($product["stock"] / 15) * 100, 100) ?>%">
            </div>
          </div>
          <div
            class="stock-message <?= $product["stock"] <= 0 ? 'out-of-stock' : ($product["stock"] > 10 ? 'high' : ($product["stock"] > 5 ? 'medium' : 'low')) ?>">
      ...
      <?php if ($product["stock"] > 0): ?>
        <div class="input-group">
          ...
          <div class="number-field">
            ...
            <input type="text" name="product-quantity" class="product-quantity" placeholder="Darab"
              max="<?= htmlspecialchars($product['stock']); ?>" min="1" value="1" pattern="[0-9]*">
      ...
      <section class="product-description">
        ...
        <div class="health-benefits">
          <header class="subtitle">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-heart-pulse"
              viewBox="0 0 16 16">
              <path
          ...
          <?php else: ?>
            <ul>
              <?php foreach ($side_effects as $index => $side_effect): ?>
                <li>
                  <b><?= htmlspecialchars($side_effect["name"]); ?></b>:
                  <?= htmlspecialchars($side_effect["description"]); ?>
                </li>
              <?php endforeach; ?>
            </ul>
          <?php endif; ?>
          ...
          <div class="tags" aria-label="Címkék">
            <?php if (is_array($tags)): ?>
              <?php foreach ($tags as $tag): ?>
                <div class="tag">
                  <img src="<?= htmlspecialchars($tag["icon_uri"]) ?>" alt="<?= htmlspecialchars($tag["name"]) ?>"
                    class="tag-icon" draggable="false" />
                </div>
              <?php endforeach; ?>
            <?php else: ?>
        ...
        <form class="review-form" action="">
          ...
          <div class="review-form-stars grey">
            <div class="star" data-index="0">
              <img src="<?= htmlspecialchars(ROOT_URL) ?>/fb-content/fb-products/media/images/star-empty.svg"
                alt="Üres csillag" class="empty active" draggable="false">
              <img src="<?= htmlspecialchars(ROOT_URL) ?>/fb-content/fb-products/media/images/star-filled.svg"
                alt="Teli csillag" class="full" draggable="false">
            </div>
                  ...
                  <picture>
                    <?php foreach ($resolutions as $index => $resolution): ?>
                      <source type="image/avif"
                        srcset="<?= $relatedProduct["thumbnail_image"] ?>-<?= $resolution ?>px.avif 1x"
                        media="(min-width: <?= $resolution ?>px)">
                      <source type="image/webp"
                        srcset="<?= $relatedProduct["thumbnail_image"] ?>-<?= $resolution ?>px.webp 1x"
                        media="(min-width: <?= $resolution ?>px)">
                      <source type="image/jpeg" srcset="<?= $relatedProduct["thumbnail_image"] ?>-<?= $resolution ?>px.jpg 1x"
                        media="(min-width: <?= $resolution ?>px)">
                    <?php endforeach; ?>
                    <!-- Fallback -->
                    <img src="<?= $relatedProduct["thumbnail_image"] ?>-<?= end($resolutions) ?>px.webp"
                      alt="<?= htmlspecialchars($relatedProduct['name']) ?>" loading="lazy"
                      sizes="(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 33vw">
                  </picture>
                  <picture class="secondary">
                    <?php foreach ($resolutions as $index => $resolution): ?>
                      <source type="image/avif"
                        srcset="<?= $relatedProduct["secondary_image"] ?>-<?= $resolution ?>px.avif 1x"
                        media="(min-width: <?= $resolution ?>px)">
                      <source type="image/webp"
                        srcset="<?= $relatedProduct["secondary_image"] ?>-<?= $resolution ?>px.webp 1x"
                        media="(min-width: <?= $resolution ?>px)">
                      <source type="image/jpeg" srcset="<?= $relatedProduct["secondary_image"] ?>-<?= $resolution ?>px.jpg 1x"
                        media="(min-width: <?= $resolution ?>px)">
                    <?php endforeach; ?>
                    <!-- Fallback -->
                    <source type="image/jpeg" srcset="<?= $relatedProduct["secondary_image"] ?>.jpg 1x"
                      media="(min-width: 0px)">
                    <img src="<?= $relatedProduct["secondary_image"] ?>.jpg"
                      alt="<?= htmlspecialchars($relatedProduct['name']) ?>" loading="lazy"
                      sizes="(max-width: 768px) 50vw, (max-width: 1200px) 33vw, 33vw">
                  </picture>
                </a>
                <div class="button-wrapper">
                  <button class="quick-add" data-product-id="<?= htmlspecialchars($relatedProduct["id"]); ?>"
              ...
              <div class="card-body">
                ...
                <?php if ($relatedProduct["review_count"] > 0): ?>
                  <div class="card-bottom">
                    <div class="review-stars stars" data-rating="<?= htmlspecialchars($relatedProduct["avg_rating"]); ?>">
                    </div>
