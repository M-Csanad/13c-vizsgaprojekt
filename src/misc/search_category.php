...
if ($searchTerm) {
    ...
    $matches = selectData("SELECT 'category' AS 'type', category.id, category.name AS `name`, 
                                   category.subname, category.description, category.thumbnail_image_horizontal_uri, 
                                   NULL AS parent_category
                           FROM category 
                           WHERE category.name LIKE ?
                           UNION 
                           SELECT 'subcategory' AS 'type', subcategory.id, subcategory.name AS `name`, 
                                   subcategory.subname, subcategory.description, subcategory.thumbnail_image_horizontal_uri, 
