...
function uploadProduct() {
    ...
    if (count(array_filter($_FILES, function ($e) { return $e['error'] == 1; })) > 0) {
        ...
        $successfulUpload = move_uploaded_file($thumbnailTmp, $productDirURI."thumbnail/".$fileName);
        if (!$successfulUpload) {
            return false;
        }
        ...
        for ($i=0; $i < $fileCount; $i++) {
            ...
            $successfulUpload = move_uploaded_file($productImageTmp, $productDirURI."gallery/".$fileName);
            if (!$successfulUpload) {
                return false;
            }
        }
